...
def money_add_in_hours():
    ...
    threading.Timer(3600, money_add_in_hours).start()
...
def text(update, context):
    ...
    # Казино
        ...
        if working != 'Безработен' and count_work_time > 0:
            ...
            default_test(update, works_menu, 'Вы и так никем не работаете')
    # Донат системы
    elif text.lower() == 'Донат':
        default_test(update, donate_menu, 'Выберите один из предложенных вариантов')

    elif text.lower() == 'Заработать донат':
        text_referal = '''
                    Вы можете заработать бабло, подписываясь на каналы
                    1-ый канал - https://t.me/testruslan1
                '''
        default_test(update, glavnoe_menu_keyboard, text_referal)
        ...
        for key in many_bussines:
            if text.lower()[:text.lower().index(',')] == key.lower() and bussines == 'Нету' and many_bussines[key]['Уровень'] <= lvl_player:
                if count_do_money >= many_bussines[key]['Стоимость']:
                    ...
                    Timer2 = threading.Timer(3600, money_add_in_hours)
                    Timer2.start()
